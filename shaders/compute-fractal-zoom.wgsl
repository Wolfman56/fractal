// This shader is functionally identical to the original compute.wgsl.
// The "fractal zoom" is achieved by how the application's Controller
// manipulates the `scale` parameter based on camera distance.
...
struct Params {
    ...
    lacunarity: f32,
    hurst: f32,
    // The 'scale' parameter is the key to fractal zoom.
    // As the user zooms in (camera gets closer), the Controller
    // should increase this value to generate higher-frequency detail.
    scale: f32,
    seed: u32,
};
...
fn perlin2d(p: vec2<f32>) -> f32 {
    let p_int = floor(p);
    let p_fract = fract(p);
    let p00 = vec2<u32>(p_int);
    let p10 = p00 + vec2<u32>(1u, 0u);
    let p01 = p00 + vec2<u32>(0u, 1u);
    let p11 = p00 + vec2<u32>(1u, 1u);
    let h00 = pcg(p00.x + pcg(p00.y + params.seed));
    let h10 = pcg(p10.x + pcg(p10.y + params.seed));
    let h01 = pcg(p01.x + pcg(p01.y + params.seed));
    let h11 = pcg(p11.x + pcg(p11.y + params.seed));
